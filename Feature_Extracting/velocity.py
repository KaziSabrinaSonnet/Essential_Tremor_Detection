    ...
    with open(path_file, 'r') as fp:
        for line in fp:
            ...
            if len(line_splits) == 5:
                ...
                ls= line_splits[0].split('-')
                if any(t ==' Lines Begun\n' for t in ls):
                    break
...
plt.plot(np.zeros(np.shape(Healthy_Velocity)), Healthy_Velocity, 'ro')
plt.plot(np.zeros(np.shape(Tremor_Velocity)), Tremor_Velocity, 'g*')
plt.tick_params(
...
for file in file_list:
    if os.path.isfile(file):
        ...
        tremor_amplitude.append(int(np.mean(trm_amp_list)))
plt.xlabel('Frequency(Hz)')
plt.xlim(right=65)
plt.yticks([])
...
#%%
    ...
    """
plt.figure(figsize=(4,6))
plt.plot(np.zeros(np.shape(peak_list_healthy[1])), peak_list_healthy[1], 'g*')
plt.plot(np.zeros(np.shape(peak_list_tremor[1])), peak_list_tremor[1], 'ro')
plt.tick_params(
axis='x',          # changes apply to the x-axis
which='both',      # both major and minor ticks are affected
bottom=False,      # ticks along the bottom edge are off
top=False,         # ticks along the top edge are off
labelbottom=False) # labels along the bottom edge are off
plt.title('Detected Peaks in Velocity Spectrogram')
plt.show()
#%%
